      ...
      <div className="flex items-center justify-between border-b p-4 dark:border-slate-700">
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end">
            ...
            <DropdownMenuItem onClick={() => navigator.clipboard.writeText(id)}>ID хуулах</DropdownMenuItem>
            <DropdownMenuSeparator />
      ...
      <div className="p-4">
        <div className="mb-4 flex justify-between text-sm">
          <div className="min-w-0">
            ...
            <p className="truncate font-medium">{format(new Date(startDate), "yyyy/MM/dd")}</p>
          </div>
          <div className="text-right min-w-0">
            <p className="text-xs text-muted-foreground">Дуусах огноо</p>
            <p className="truncate font-medium">{format(new Date(endDate), "yyyy/MM/dd")}</p>
          </div>
        </div>
        <div className="flex items-center justify-between">
          <div className="flex -space-x-2 overflow-hidden">
            {assignees.slice(0, 4).map((assignee, index) => (
              <Avatar
                key={assignee.id}
                className={cn("h-8 w-8 border-2 border-background dark:border-slate-800", `bg-${assignee.color}-500`)}
              >
                <AvatarFallback className={cn(`bg-${assignee.color}-500 text-white font-medium`)}>
          ...
          {audioUrl && (
            ...
            >
              ...
              <span className="sr-only">{isPlaying ? "Зогсоох" : "Тоглуулах"}</span>
            </Button>
          )}
        </div>
      </div>
